CREATE PROCEDURE WSO2_CONFIRMATION_CODE_CLEANUP AS
BEGIN
    ...
    BEGIN
        -- CREATE CHUNK TABLE
        IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'IDN_RECOVERY_DATA_CHUNK_TMP'))
        DROP TABLE IDN_RECOVERY_DATA_CHUNK_TMP;
        ...
        BEGIN
            -- CREATE BATCH TABLE
            IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'IDN_RECOVERY_DATA_BATCH_TMP'))
            DROP TABLE IDN_RECOVERY_DATA_BATCH_TMP;
